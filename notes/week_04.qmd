...
To understand the statistical properties of the local constant estimator, we analyze its **bias** and **variance**. This will allow us to study its mean squared error and to discuss bandwidth selection.

## 1.1 Decomposition

Write $Y_i = m(x) + \big(m(X_i)-m(x)\big) + \varepsilon_i$, with $\mathbb{E}[\varepsilon_i|X_i]=0$, $\mathbb{E}[\varepsilon_i^2|X_i]=\sigma^2$. Plugging this into the estimator gives

$$
\widehat{m}(x) = m(x) + \frac{\widehat{m}_1(x)}{\widehat{f}(x)} + \frac{\widehat{m}_2(x)}{\widehat{f}(x)},
$$
...
$$
\widehat{m}_1(x) = \frac{1}{nh}\sum_{i=1}^n K\!\left(\frac{X_i-x}{h}\right)\big(m(X_i)-m(x)\big),
$$

$$
\widehat{m}_2(x) = \frac{1}{nh}\sum_{i=1}^n K\!\left(\frac{X_i-x}{h}\right)\varepsilon_i.
$$

Thus, $\widehat{m}_1$ captures the contribution of the local slope and curvature of $m$, while $\widehat{m}_2$ captures the stochastic noise.

---

## 1.2 Expectation of $\widehat{m}_2(x)$

Since $\mathbb{E}[\varepsilon_i|X_i]=0$, we immediately obtain

$$
\mathbb{E}[\widehat{m}_2(x)] = 0.
$$

The variance is:

$$
\mathrm{Var}(\widehat{m}_2(x)) = \frac{\sigma^2}{nh^2}\,\mathbb{E}\!\left[K\!\left(\frac{X-x}{h}\right)^2\right].
$$

Changing variables $u=(z-x)/h$ gives

$$
\mathrm{Var}(\widehat{m}_2(x)) = \frac{\sigma^2}{nh}\,f_X(x) g(K) + o\!\left(\frac{1}{nh}\right),
$$

with $g(K) = \int K(u)^2 , du$.

---

## 1.3 Expectation of $\widehat{m}_1(x)$

Expanding $m(X_i)$ around $x$ with a Taylor series:

$$
m(x+hu) = m(x) + hu\,m'(x) + \tfrac{1}{2}(hu)^2 m''(x) + o(h^2).
$$

Multiplying with the expansion of $f_X(x+hu)$ and integrating against $K(u)$, the $O(h)$ terms vanish because $\int uK(u)du=0$. The leading term is of order $h^2$:

$$
\mathbb{E}[\widehat{m}_1(x)] = h^2 \sigma_K^2\left( \tfrac{1}{2} m''(x) f_X(x) + m'(x) f'_X(x) \right) + o(h^2),
$$

where $\sigma_K^2 = \int u^2 K(u),du$.

The variance of $\widehat{m}_1(x)$ is of smaller order, $O(h/n)$, and negligible compared to its mean.

---

## 1.4 Bias and Variance of $\widehat{m}(x)$

Combining the pieces and using $\widehat{f}(x) \to f_X(x)$:

* **Bias:**

$$
\mathbb{E}[\widehat{m}(x)] - m(x) \;\approx\; h^2 \sigma_K^2 \left( \tfrac{1}{2} m''(x) + m'(x) \frac{f'_X(x)}{f_X(x)} \right).
$$

* **Variance:**

$$
\mathrm{Var}(\widehat{m}(x)) \;\approx\; \frac{\sigma^2 g(K)}{n h f_X(x)}.
$$
...
* The **bias** is of order $o(h^2)$ and depends on both the curvature of $m$ and the slope of $m$ relative to the design density $f_X$.
* The **variance** is of order $o\left((nh)^{-1}\right)$.
