...
**Leading Ex.:** The mean $\theta = \E X_1 = \int x d F(x)$. [$\Delta$]{style="float:right"}
...
**Leading Ex.:** The empirical estimator of the mean is exactly the mean of the empirical distribution $\widehat{\theta} := \frac{1}{N} \sum_{n=1}^N X_n = \int x d \widehat{F}_N(x)$. [$\Delta$]{style="float:right"}
...
**Leading Ex.:** For $F$ a distribution function of the Gaussian distribution with variance $\sigma^2$, $\widehat{\theta} \sim \mathcal{N}(\theta,\frac{\sigma^2}{N})$, which is the sampling distribution. [$\Delta$]{style="float:right"}
...
$$
where $z(1-\alpha)$ is the $(1-\alpha)$-quantile of the standard Gaussian. By the [Berry-Esseen theorem](https://en.wikipedia.org/wiki/Berry%E2%80%93Esseen_theorem):
$$
...
$$
for all $x \in \R$ and $\Phi$ and $\phi$ being respectively the distribution function and the density of the standard Gaussian, while the bootstrap version satisfies
$$
...
**Example:** Assume $X_1,\ldots,X_N$ are i.i.d. with $\E |X_1|^3 < \infty$ and $\E X_1 = \mu$, and let $\theta = \mu^3$. In the same spirit to the leading example above, the empirical estimator of $\theta$ is $\widehat{\theta} = (\bar{X}_N)^3$, which is biased:
...
$$
\E_F \widehat{\theta} = \E_F\bar{X}_N^3 = \E_F\big[ \mu + N^{-1} \sum_{n=1}^N(X_n - \mu) \big]^3 = \mu^3 + \underbrace{N^{-1} 3 \mu \sigma^2 + N^{-2} \gamma}_{=b},
$$
...
$$
\E_F \widehat{\theta}_b^\star = \mu^3 + N^{-1} 3 \big[ \mu \sigma^2 - \E_F \bar{X}_N \widehat{\sigma}^2 \big] + N^{-2} \big[ \gamma - \E_F \widehat{\gamma} \big].
$$

At this point, there are several ways to argue that the bias has been decreased (but all of them are a bit painful). For example, it can be computed explicitly that $\E \bar{X}_N \widehat{\sigma}^2 = \mu \sigma^2 + \mathcal{O}(N^{-1})$ and $\E \widehat{\gamma} = \gamma + \mathcal{O}(N^{-1})$, and hence $\text{bias}(\widehat{\theta}_b^\star) = \mathcal{O}(N^{-2})$ as opposed to $\text{bias}(\widehat{\theta}) = \mathcal{O}(N^{-1})$. Thus, using bootstrap, we have successfully reduced the bias by the full order of the sample size. [$\Delta$]{style="float:right"}
...
* the estimator is given as $\widehat{\theta} = \theta(\widehat{F})$
  - we will also write $\widehat{\theta} = \theta[\mathcal{X}]$, since $\widehat{F}$ and thus the estimator depends on the sample
...
For example, the quantiles $q^\star(\alpha)$ in [the leading example above](#running_ex) can be approximated by the quantiles of this empirical distribution.
...
Any statistical methodology should be at least consistent. If we have consistency, we may ask for rates of convergence, like sketched in [the example above](#running_ex). But with bootstrap, theoretical research is hard, results have to derived again for different kinds of statistics. Here we will only provide a consistency result for the case of bootstrapping a smooth transformation of the empirical mean.
...
Notice that while (parametric and non-parametric) bootstrap rejects the hypothesis on 5 % level, the test based on asymptotic normality does not. This could be expected, since we saw in our [leading example](#running_ex) above that bootstrap produces better one-sided CIs, which are naturally dual to one-sided hypothesis tests.
<!-- On the other hand, it can be shown that in the case of two-sided CIs and two-sided tests the asymptotic behavior of asymptotic normality is better, matching the bootstrap rates, and hence bootstrap is not expected to outperform the asymptotic test. As shown below, it is in fact the other way around. Note that we reduce the value of $\mu_0$ below to gain more power, since it is naturally harder to reject against the two-sided alternative. -->
...
$$
\begin{split}
\E (\widehat{b}) &= (N-1)\big\lbrace\text{bias}(\bar{\theta}) - \text{bias}(\widehat{\theta}_{})\big\rbrace = (n-1)\left\lbrace a \left( \frac{1}{N-1} - \frac{1}{N} \right) + \left( \frac{1}{(N-1)^2} - \frac{1}{N^2} \right) + \mathcal{O}\left(\frac{1}{N^3}\right)\right\rbrace \\&= a N^{-1} + b N^{-2} \frac{2N-1}{N-1} + \mathcal{O}(N^{-3})
\end{split},
$$
